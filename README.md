...
腳本檔：`zh-tw-webui.user.js`（v3.6.0）
...
字典規模：claude.ai 559 條 + 48 條規則；github.com 360 條 + 30 條規則。
...
- **盤點漏翻**：按 `Ctrl + Alt + D`，會把這一頁**真的還沒翻到**的介面字串複製到剪貼簿，
...
不同頁面要分開按（首頁、專案頁、設定頁、GitHub 的議題頁各按一次），因為每頁的介面元素不一樣。
