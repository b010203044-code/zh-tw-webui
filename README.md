...
- **讀回存檔時會用「現在這一版」的規則重篩一遍（v3.17.0 新增）**。腳本更新後，
  上一版記下的字串有些已經補進字典、有些已經改成自動過濾，不重篩就會一直留在存檔裡，
  匯出時看起來像「補了也沒變少」。重篩用的 `stillMissing()` 跟 `recordMiss()` 是同一套判斷，
  篩掉幾條會印在主控台。所以**更新腳本之後匯出，清單會自己變短**，不必手動清。
...
| 主控台貼不上去 | Chrome／Edge 的自我保護。先按 `Ctrl + V` 試貼一次，畫面會跳出警告，**照警告裡那句話指定的字打**——中文介面是 `允許貼上`，英文介面才是 `allow pasting`，打錯會噴 `SyntaxError`。打完按 Enter 再重新貼一次就成功了，只要做一次 |
...
- **版本號開頭的字串**（`v3.11.0 commit`，v3.22.0 新增的過濾）是你自己的紀錄，不是介面文字。
